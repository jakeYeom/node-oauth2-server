...
AuthorizeHandler.prototype.getClient = function(request) {
  ...
  if (redirectUri && !is.uri(redirectUri)) {
    throw new InvalidRequestError('Invalid request: `redirectUri` is not a valid URI');
  }
  return promisify(this.model.getClient, 2).call(this.model, clientId, null)
    .then(function(client) {
      ...
      if (!_.includes(client.grants, 'authorizationCode')) {
        throw new UnauthorizedClientError('Unauthorized client: `grantType` is invalid');
      }

      if (!client.redirectUri || 0 === client.redirectUri.length) {
        throw new InvalidClientError('Invalid client: missing client `redirectUri`');
      }

      if (redirectUri && !_.includes(client.redirectUri, redirectUri)) {
...
AuthorizeHandler.prototype.getRedirectUri = function(request, client) {
  return request.body.redirectUri || request.query.redirectUri || client.redirectUri;
};
