...
RefreshTokenGrantType.prototype.getRefreshToken = function(request, client) {
  ...
  return promisify(this.model.getRefreshToken, 1).call(this.model, request.body.refreshToken)
    .then(function(token) {
      if (!token) {
      ...
      if (token.refreshTokenExpiresAt && token.refreshTokenExpiresAt < new Date().getTime()) {
